...
img1 = imread('inset.jpg');
img1=img1(101:300, 101:300, :);
figure, imshow(img1);
img=blurimage(img1);
img=uint8(img);
%Show input image
...
figure
img2=double(img1(:,:,1));
surf(log10(abs(fft2(img2))));
s1=norm(reshape(fft2((img2)),numel(img2),1),1)
shading flat;
view(0, 90);
figure
img3=double(img(:,:,1));
surf(log10(abs(fft2(img3))));
s2=norm(reshape(fft2((img3)),numel(img3),1),1)
